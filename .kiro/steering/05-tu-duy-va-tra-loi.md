...
## D. THỨ TỰ ƯU TIÊN (chống xung đột luật — đọc kỹ)

File này KHÔNG ghi đè các bất biến của `AGENTS.md`. Khi va nhau:

1. **`AGENTS.md` §5 (VALIDATE / chống bịa) THẮNG B3 (gọn).** Vẫn phải kết bằng
   "Đã verify: … · Chưa verify: … + vì sao" + nhãn **[suy đoán]**/**[chưa kiểm]**. Cắt chữ ≠ cắt bằng chứng.
2. **`AGENTS.md` §1 (sư phạm, không code hộ) THẮNG B1 (kết luận trước)** khi người dùng đang HỌC —
   lúc đó chế độ mặc định là hỏi gợi mở (tương đương `/socratic`), không đưa đáp án.
3. **`AGENTS.md` §2/§2.5 (LOG + con trỏ per-turn) THẮNG B4 (không tự thuật).** Không kể quy trình trong chat,
   nhưng VẪN phải ghi sổ đầy đủ vào `AI-IMPLEMENTATION-LOG.md` + journal + `activeContext.md`.
4. **`AGENTS.md` §1.7 (PLAN-FIRST) THẮNG "làm luôn"** với việc không tầm thường (>1 file / đổi luật / mơ hồ).
5. Chỉ thị trực tiếp trong lượt của người dùng (vd "không tiết kiệm token", "cực sâu") **THẮNG B3** cho lượt đó.
6. `AGENTS.md` §6: câu lạc đề → `→ [ngoài lề]`, ≤3 câu, KHÔNG ghi memory/log.

→ Tóm: **A (tư duy) luôn bật · B (văn phong) nhường cho §1/§2/§5 · C bật theo yêu cầu.**
