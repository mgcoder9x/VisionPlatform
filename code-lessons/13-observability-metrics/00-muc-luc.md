...
| 08 | `MetricsHttpExporter` — `http.server` daemon PULL `/metrics`, secure-default localhost, 500-không-sập | `adapters/metrics_http_server.py` | ⬜ |
| 09 | `_serving` Event chống DEADLOCK `stop()` (K-071: shutdown cần serve_forever đang chạy) | `adapters/metrics_http_server.py` | ⬜ |
| 10 | Wiring: 1 `InMemoryMetrics` + 1 exporter DÙNG CHUNG → aggregate theo `source_id` (1 process/camera → 1 scrape target) | `vision_slice_app::_build_config_observability` | ⬜ |
