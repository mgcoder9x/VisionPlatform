...
|---|---|---|---|
| 01 | Analytics STATEFUL vs detect STATELESS — vì sao đếm-không-trùng cần định-danh-xuyên-khung | `runtime/stages/tracking_stage.py` vs `count_stage.py` | ⬜ |
| 02 | `domain/tracking.py::greedy_associate` — vì sao INDEX-based (domain cấm import kernel `Detection`) | `domain/tracking.py` | ⬜ |
| 03 | IoU-greedy + tie-break XÁC ĐỊNH `sort(-iou, ni, pi)` — test lặp-lại-được | `domain/tracking.py` | ⬜ |
| 04 | `runtime/iou_tracker.py` — `_TrackState` mutable, `update()` 6 bước, `unique_count=_next_id` đơn điệu | `runtime/iou_tracker.py` | ⬜ |
| 05 | `Track` DTO (frozen) — `track_id`/`age`/`hits`; box giữ `space` (invariant Step 02) | `kernel/tracking_protocol.py` | ⬜ |
| 06 | `TrackingStage` — đọc artifacts['detections'], camera-affinity fail-fast, ghi tracks/unique/active | `runtime/stages/tracking_stage.py` | ⬜ |
| 07 | `domain/geometry.py::orient` — cross-product dấu = "phía nào của đường" | `domain/geometry.py` | ⬜ |
| 08 | `segments_intersect` — proper intersection; collinear/điểm-suy-biến = False (vì sao) | `domain/geometry.py` | ⬜ |
