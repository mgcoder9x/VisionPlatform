...
### Entry #366 — 2026-07-13 — Mở sub-spec `batch-mux` (design-first): thiết kế gộp N-cam→1 session.run nâng C_inf (+D-100, +K-093) — Kiro-Opus
**Bối cảnh:** user chọn hướng (a) batch-mux design-first (roadmap #3 scale-architecture, đòn bẩy C_inf lớn nhất). Có baseline K-092 (~105/s@K4 K-session-rời) làm mốc so sánh nghiệm thu. Đọc CODE THẬT đường infer + CHẠY THẬT probe model TRƯỚC khi viết design (không bịa).
**1. Quyết định AI tự ra (+D-100):** tạo `.kiro/specs/batch-mux/design.md` — thiết kế BatchMuxer (gather tới max_batch HOẶC batch_timeout → preprocess_batch stack [B,3,640,640] → 1 session.run → postprocess_batch split → scatter theo request_id). Port RIÊNG `IBatchDetector` (không sửa `IDetector` single-frame — backward-compat). 5 Correctness Property (identity/latency-bounded/shed/tương-đương-single-vs-batch/nghiệm-thu-benchmark). Self-review 6 lỗ. KHÔNG code (design-first, chờ valid).
**2. Chỗ phải đổi (+K-093 — VERIFY bằng chạy thật):** phát hiện model `yolov8n.onnx` hiện tại input CỐ ĐỊNH `[1,3,640,640]` (probe `session.run` batch=2/4 → `InvalidArgument: Got 2 Expected 1`) → batch-mux BẤT KHẢ THI nếu không re-export model với trục batch ĐỘNG (`dynamic=True`). Thành ràng buộc RB-1 + task #0 pha thi công. Đây là chỗ "yêu cầu ban đầu (gộp batch)" phải đổi: cần điều-kiện-tiên-quyết re-export.
**3. Trade-off:** self-viết BatchMuxer (kiểm soát/nhẹ/no-docker) vs Triton (dynamic-batch sẵn nhưng ops nặng, máy no-docker khó) → nghiêng tự-viết. throughput↔latency (batch_timeout) = đánh đổi cốt lõi, phải đo. NGHI VẤN LỚN (Lỗ 1, [chưa kiểm]): batch-mux có thể KHÔNG thắng K-session-rời vì yolov8n nhỏ + GPU đã lấp khá đầy (K-092 aggregate 2.25x) → nghiệm thu = ĐO, nếu không vượt 104.7/s thì kết luận trung thực "không đáng".
**4. Điều bạn nên biết:** design bám 5 ràng buộc ĐÃ VERIFY (RB-1..5, đọc code + chạy probe). Bước rẻ nhất kế: spike đo bench (re-export dynamic + đo B=1/2/4/8 vs baseline) TRƯỚC khi build BatchMuxer đầy đủ — trả lời Lỗ-1. Design-first PHA-1, chưa đủ thi công (cần requirements.md + tasks.md dẫn xuất).
**Đã verify:** probe model batch cố định (output `InvalidArgument` thật); đọc code IDetector/OnnxDetector/DetectorPipeline (single-frame, batch=1 cứng); design.md getDiagnostics = 0 (format spec sạch); temp `_tmp_probe_batch.py` dọn; tree sạch trước ghi sổ. · **Chưa verify:** batch-mux có thắng K-session-rời không (cần re-export + bench); requirements.md/tasks.md chưa tạo.

### Entry #367 — 2026-07-13 — Batch-mux: tạo requirements.md (design-first, dẫn xuất từ design đã valid) — Kiro-Opus
**Bối cảnh:** user đã đọc-lại-valid `batch-mux/design.md` → OK → tiếp design-first: Design → Requirements → Tasks. Tạo requirements.md EARS bám 5 property + ràng buộc RB-1..5 của design.
**1. Quyết định AI tự ra:** `.kiro/specs/batch-mux/requirements.md` — 5 Requirement EARS: R1 đúng-đắn batch (1.1 identity không-trộn-cam / 1.2 tương-đương-single-vs-batch / 1.3 coordinate per-sample), R2 latency-bounded + shed + bulkhead (2.1..2.4), R3 nghiệm-thu-bằng-đo vượt baseline K-092 (3.1..3.3), R4 backward-compat không phá lõi (4.1..4.3), R5 tiên-quyết-dynamic-batch fail-fast (5.1..5.2). Số criteria KHỚP `Validates` trong design.md (1.1/1.2/2.1/2.2/3.1). Non-Goals + DoD + Glossary đầy đủ.
**2. Chỗ phải đổi:** không (dẫn xuất trực tiếp từ design đã valid; thêm R4/R5 tường minh hoá ràng buộc RB đã có trong design).
**3. Trade-off:** requirements nêu rõ nghiệm thu = ĐO + kết luận "không đáng" là hợp lệ (R3.2) — chống sunk-cost. Không chốt Triton-vs-tự-viết / concurrency (Non-Goal, để design/tasks).
**4. Điều bạn nên biết:** đây là PHA requirements design-first, CHƯA code, CHƯA tasks.md. Bước kế = user valid requirements → tasks.md (chia task TDD, task-0 = re-export model dynamic + spike bench trả lời Nghi-vấn Lỗ-1).
**Đã verify:** requirements.md getDiagnostics = 0 (format spec sạch, heading chuẩn); số criteria khớp Validates trong design; drift chạy kế. KHÔNG đổi code (647/2 giữ). · **Chưa verify:** tasks.md chưa tạo; bench batch-mux chưa chạy.

### Entry #368 — 2026-07-13 — Batch-mux: tạo tasks.md (khép bộ spec Design→Requirements→Tasks) — Kiro-Opus
**Bối cảnh:** user valid requirements OK → tạo tasks.md khép bộ spec batch-mux. Đặt **Task 0 = SPIKE BENCH làm CỔNG QUYẾT ĐỊNH** (theo yêu cầu user + R3.2 chống sunk-cost).
**1. Quyết định AI tự ra:** `.kiro/specs/batch-mux/tasks.md` — 5 task (0-4): **Task 0 spike bench** (re-export model dynamic + đo B=1/2/4/8 vs baseline K-092 104.7/s → GATE: không vượt thì DỪNG, không build tiếp); Task 1 preprocess_batch/postprocess_batch thuần + verify bằng model tí-hon dynamic (no-GPU/no-network, R5.2); Task 2 IBatchDetector port + BatchOnnxDetector fail-fast dynamic-check; Task 3 BatchMuxer gather-scatter + timeout + shed + bulkhead (event-driven test); Task 4 PBT + regression. Task Dependency Graph (wave 0→4, Task 0 là GATE nhánh DỪNG). Mỗi task ref requirement cụ thể.
**2. Chỗ phải đổi:** không (dẫn xuất design+requirements đã valid). Nhấn mạnh Task 0 là cổng — KHÔNG tự động sang Task 1 (khác các spec trước làm tuyến tính).
**3. Trade-off:** Task 0 cần network (re-export) + GPU (đo) nhưng đặt TRƯỚC để trả lời Nghi-vấn Lỗ-1 rẻ nhất; Task 1-3 thuần logic no-GPU (model tí-hon dynamic) làm được độc lập. Đánh đổi: spike trước = biết sớm có đáng không, tránh build phí.
**4. Điều bạn nên biết:** BỘ SPEC batch-mux HOÀN CHỈNH (design + requirements + tasks, đều 0 diagnostic, design-first). CHƯA code dòng nào. Bắt đầu thi công = chạy Task 0 (cần đèn xanh re-export model dynamic — network). Nếu Task 0 gate FAIL → spec đóng ở mức spike, kết luận trung thực.
**Đã verify:** tasks.md getDiagnostics=0; mỗi task ref requirement; Task Dependency Graph JSON+mermaid hợp lệ; drift chạy kế. KHÔNG đổi code (647/2 giữ). · **Chưa verify:** chưa chạy task nào; bench batch-mux chưa đo.

### Entry #369 — 2026-07-13 — REVIEW đối kháng spec batch-mux (đọc-lại-valid) → fix 3 lỗ bản chất trước khi code (+D-101, +K-094) — Kiro-Opus
**Bối cảnh:** đúng nguyên tắc user "chuẩn bị design → ĐỌC LẠI VALID kiểm-chứng-được → mới triển khai". Viết spec 1 mạch (#366-368) nên tự PHÁ nó trước khi code. Đọc CODE THẬT `TrackingStage` + `IouTracker` (không suy đoán) để kiểm mắt xích nghi ngờ nhất: batch-mux gộp nhiều-cam có mâu thuẫn camera-affinity/analytics-stateful không.
**1. Quyết định AI tự ra (+D-101):** review đối kháng → tìm 3 lỗ + vá vào design.md/requirements.md: (a) mục MỚI "Batch-mux ↔ analytics CÓ TRẠNG THÁI" — mux ở ranh giới `IDetector` STATELESS (thượng nguồn stage stateful), scatter giữ affinity K-042; (b) **Property 6** (thứ tự frame per-camera bảo toàn qua mux) + **R1.4**; (c) siết Property 2 (latency đủ chuỗi queue_wait+gather+pre+infer+post, không chỉ timeout+infer) + Lỗ 7 self-review. getDiagnostics=0 cả 2 file.
**2. Chỗ phải đổi (+K-094 — VERIFY đọc code):** `IouTracker.update()` PHỤ THUỘC THỨ TỰ (mỗi call `age += 1` toàn bộ track + associate frame-trước) + `TrackingStage._source_id` guard cứng (2 source_id → raise). ⇒ batch-mux (gộp cross-camera) PHẢI: mux tại tầng stateless + bảo toàn thứ tự frame per-camera downstream. Design ban đầu (Property 1 chỉ phủ "không lẫn cam") BỎ SÓT ràng buộc THỨ TỰ → đã bổ sung Property 6.
**3. Trade-off:** gather-infer TUẦN TỰ (1 session/1 thread) → thứ tự bảo toàn tự nhiên (đơn giản, đúng); nếu sau này nhiều mux-worker song song (tăng throughput) → BẮT BUỘC re-order per-camera (phức tạp hơn) — ghi rõ trong design để không quên.
**4. Điều bạn nên biết:** lỗ này BẢN CHẤT — nếu bỏ qua, batch-mux chạy "đúng" cho detect nhưng làm HỎNG tracking/đếm khi bật analytics stateful downstream (bug ngầm khó tìm). Bắt được TRƯỚC code = giá trị của "đọc lại valid". Spec batch-mux giờ vững hơn; vẫn CHƯA code (Task 0 spike chờ đèn xanh network).
**Đã verify:** đọc `TrackingStage`/`IouTracker` thật (stateful + order-dependent, có bằng chứng code); design.md + requirements.md getDiagnostics=0 sau vá; drift chạy kế. KHÔNG đổi code sản phẩm (647/2 giữ). · **Chưa verify:** Property 6 chưa có test (thuộc Task 3/4); bench batch-mux chưa chạy.

### Entry #370 — 2026-07-13 — VERIFY khả thi chiến lược test Task 1 (model tí-hon dynamic-batch, R5.2) — không network (+K-095) — Kiro-Opus
**Bối cảnh:** spec batch-mux (R5.2) KHẲNG ĐỊNH "verify logic mux bằng model ONNX tí-hon dynamic tự tạo, không cần YOLO/GPU/network". Đây là GIẢ ĐỊNH chưa kiểm trong chính spec → đúng nguyên tắc "không bịa, verify" phải kiểm TRƯỚC khi dựa vào (de-risk test strategy). Không phạm cổng Task 0 (không build sản phẩm, chỉ kiểm test infra).
**1. Quyết định AI tự ra (+K-095):** probe read-only → `onnx` builder CÓ sẵn (torch vắng nhưng không cần) → dựng model `ReduceSum` input `['N',3,4,4]` (trục 0 ĐỘNG) → onnxruntime chạy batch=1/2/4 THẬT: shape đúng, giá trị đúng, **identity đúng** (đảo thứ tự sample → output đảo tương ứng = không lẫn sample). Đánh dấu R5.2 ✅ VERIFIED trong design.md + requirements.md.
**2. Chỗ phải đổi:** không (xác nhận giả định spec đúng). Gỡ 1 "unverified assumption" khỏi spec.
**3. Trade-off:** kiểm bằng temp script 1-lần (`_tmp_probe_tinydyn.py`, đã xóa) — thăm-dò hợp lệ (§3.1 cho phép 1-lần), KHÔNG thêm vào launcher (chưa phải logic lặp; sẽ thành test thật ở Task 1).
**4. Điều bạn nên biết:** cột sống test Task 1 (identity/tương-đương Property 1/4) giờ CHẮC CHẮN làm được không cần network/GPU. Khi thi công Task 1 chỉ cần port công thức K-095 thành fixture test. Task 0 (throughput, cần network) vẫn là cổng riêng.
**Đã verify:** chạy thật model tí-hon batch=1/2/4 (output: shape_ok/val_ok/identity_ok=True cả 3); temp dọn; tree sạch; drift chạy kế. KHÔNG đổi code sản phẩm (647/2 giữ). · **Chưa verify:** batch-mux throughput (Task 0, network); chưa code Task 1.

### Entry #371 — 2026-07-13 — REVIEW #371 batch-mux ↔ backpressure/transport THẬT → chốt điểm tích hợp cross-process (+D-102, +K-096) — Kiro-Opus
**Bối cảnh:** đọc-lại-valid tiếp: batch-mux tương tác thế nào với backpressure 2-tầng + transport ZMQ HIỆN CÓ? Đọc CODE THẬT `BoundedQueue` (K-016), `camera_worker` (bound-before-send 2-tầng), `InferenceServer.serve` (xử lý one-at-a-time) — không suy đoán.
**1. Quyết định AI tự ra (+D-102):** vá design.md + requirements.md + tasks.md: (a) mục MỚI "Điểm tích hợp THẬT cross-process" — batch-mux nằm TẠI `InferenceServer.serve` (điểm hội tụ ZMQ duy nhất), scatter theo ZMQ `ident`, gather = recv-multipart poll-timeout; (b) CHỐT Open Decision "đứng riêng vs tích hợp server" = tích hợp server; (c) +Task 4 "WIRE vào InferenceServer.serve" (đổi loop recv-1→detect-1 thành gather→detect_batch→scatter, additive cờ batch, batch=1=cũ) + R4.4; cập nhật Task Dependency Graph (wave 0-5).
**2. Chỗ phải đổi (+K-096 — VERIFY đọc code):** hình vẽ design ban đầu ("in-process inbound queue + camera submit") là MÔ HÌNH TEST, KHÔNG phải deployment thật. Thật: camera = process RIÊNG → ZMQ → server. ⇒ (i) KHÔNG gộp cross-camera bằng BoundedQueue (thread-safe≠process-safe, K-016); (ii) gộp phải ở `InferenceServer` (1 process); (iii) scatter = ZMQ ident (ROUTER route sẵn, mạnh hơn map tay); (iv) backpressure camera-side (SHM+window) TRỰC GIAO, không trùng-đếm.
**3. Trade-off:** tích hợp server (tái dùng transport/SHM/switchover) vs đứng riêng (phải tái tạo transport = thừa) → chọn tích hợp. Giữ Task 3 (BatchMuxer LOGIC in-process) để TEST thuần + Task 4 (wire ZMQ) tách riêng → test logic không cần ZMQ, wire test cross-process riêng (2 tầng test rõ ràng).
**4. Điều bạn nên biết:** batch-mux giờ có ĐIỂM TÍCH HỢP grounded (đọc code), không mơ hồ. Property 1 (identity) được ZMQ ident hậu thuẫn (mạnh hơn map thủ công). Đây là lỗ "chỗ nào để lắp" — nếu bỏ qua dễ phạm K-016 (BoundedQueue cross-process) = hỏng dữ liệu ngầm. Vẫn CHƯA code (Task 0 chờ đèn xanh).
**Đã verify:** đọc `BoundedQueue`/`camera_worker`/`InferenceServer.serve` thật (one-at-a-time + K-016 + 2-tầng, có code); 3 file spec getDiagnostics=0 sau vá; drift chạy kế. KHÔNG đổi code sản phẩm (647/2 giữ). · **Chưa verify:** batch-mux throughput (Task 0); wire server (Task 4) chưa code.

### Entry #372 — 2026-07-13 — Cập nhật end.md handoff chuyển máy (frontier #371, batch-mux spec) — Kiro-Opus
**Bối cảnh:** user chuyển máy → yêu cầu lưu việc đã-làm/cần-làm vào `end.md` rồi push. end.md cũ STALE (#347, nội dung Z1/R1 + lẫn đoạn chat dán) → viết lại sạch khớp frontier #371.
**1. Quyết định AI tự ra:** viết lại `end.md` 8 mục: §0 đầu-phiên · §1 trạng thái (#371, 647/2, Σ252, HEAD 26c5bec, GPU no-docker) · §2 cơ chế vận hành (vp verify/check, ghi-sổ, K-085 diff-stat, PowerShell workaround, network chờ-đèn-xanh) · §3 chống-drift 4 lớp (+ cảnh báo "#0"→C7 fail) · §4 đã-làm #365-371 · §5 TRỌNG TÂM sub-spec batch-mux (D-100..102, K-093/095/096, 3 vòng đọc-lại-valid, Task 0 gate, Nghi-vấn Lỗ-1) · §6 hướng tiếp (Task 0 cần đèn xanh network) · §7 chặn (docker/network/model-dynamic/license AGPL) · §8 file quan trọng.
**2. Chỗ phải đổi:** end.md cũ hoàn toàn lỗi thời (24 entry trước) → thay mới (không giữ nội dung Z1/R1 cũ vì đã đóng + đã có trong journal).
