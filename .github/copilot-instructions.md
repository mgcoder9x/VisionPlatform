...
4. **Cập nhật bộ nhớ (con trỏ LUÔN MỚI):** cuối MỖI lượt có đổi trạng thái → cập nhật NGAY con trỏ khu đang làm (`activeContext.md` + tracker đang dùng: `implement/00-IMPLEMENTATION-TRACKER.md` hoặc `lessons/00-LEARNING-MAP.md`) + mốc "Cập nhật lúc" (write-ahead). Sau mỗi mốc/cuối phiên: thêm `progress.md`.
...
6. **Import 6 layer** (domain/kernel/runtime/application/adapters/profiles) — không vi phạm.
