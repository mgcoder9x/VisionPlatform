...
| D-059 | 🔵 | SẢN PHẨM — spec `object-tracking-count` PHA1 design-first (2 file 0-diag): analytics STATEFUL đầu tiên (tracking IoU-greedy + đếm-không-trùng), đóng Lỗ 3/K-042. 3 lớp domain(greedy_associate)/kernel(Track/ITracker)/runtime(IouTracker/TrackingStage), camera-affinity fail-fast, no-GPU test. Additive (không sửa CountStage). CHỜ user valid → code TDD | #258 |
