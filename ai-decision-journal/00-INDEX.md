...
| D-071 | 🔵 design-only | Mở spec `metrics-exposition` (PHA1): phơi `InMemoryMetrics` → Prometheus text format 0.0.4 (counter+gauge) cho ~100 cam đa-tiến-trình scrape được. Renderer THUẦN `render_prometheus` @adapters (nhận DTO thuần, leaf) + accessor `iter_metrics()` additive @runtime (fix GỐC lossy: dữ-liệu-cấu-trúc thay parse chuỗi key `name{k=v}`) + DTO `MetricSample` @kernel. Hand-roll (không prometheus_client, T-026). Histogram bucket + HTTP `/metrics` = Non-Goal/follow-on. **0 diagnostic (get_diagnostics 2 file)** | #279 |
