...
| D-060 | 🔵 | SẢN PHẨM — spec `line-crossing-count` PHA1 design-first (2 file 0-diag): đếm vật QUA VẠCH theo hướng (in/out/total), analytics tầng-2 trên tracking. geometry thuần domain (cross-product/segments_intersect) + LineCrossingStage runtime (stateful, camera-affinity, prune bounded-memory). Additive. CHỜ user valid → code TDD | #261 |
...
| K-060 | 🟡 | IouTracker v1 greedy IoU: giới hạn cross-over (2 vật giao nhau có thể HOÁN id — nâng cấp ML qua ITracker port); camera-affinity 1-instance/1-camera (trộn→ERROR); unique_count đơn điệu; Non-Goal: line-crossing/cross-process-state/re-ID; CHƯA wire profile `--track` | #259 |
