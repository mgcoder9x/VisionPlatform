...
| D-061 | 🔵 | SẢN PHẨM — spec `crossing-event-log` PHA1 design-first (2 file 0-diag): biến đếm→SỰ KIỆN JSONL bền vững (audit/tích hợp). CrossingEvent DTO@kernel + sửa additive LineCrossingStage (phát crossing_events + clock tiêm) + CrossingEventJsonlSink@adapters + wire `--crossing-out`. CHỜ user valid → code TDD | #263 |
...
| K-061 | 🟡 | LineCrossingStage v1: prune-bounded-memory ĐỔI track-nhấp-nháy-reset-mốc (sót 1 lượt); collinear=không cắt (đi dọc≠qua); 1 vạch/instance; in/out phụ thuộc thứ tự (A,B); `--line` cần `--track`; chưa CrossingEvent DTO | #262 |
